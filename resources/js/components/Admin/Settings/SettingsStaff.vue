          ...
          <table class="table table-hover">
            ...
            <tbody>
              <tr v-for="d in data" :key="d.id">
                ...
                <td>{{ d.is_vacation | isRestVacation}}</td>
                <td>
          ...
          <form @submit.prevent="editMode ? updateRank() : createData()">
            <div class="modal-body">
              <div class="form-group">
                ...
                >{{errormsg(form.errors.get('full_name'),"full name","スタッフ名")}}</div>
              </div>
              <div class="form-group">
                <label>表記名</label>
                <input
                ...
                <div
                  ...
                  class="invalid-feedback"
                >{{errormsg(form.errors.get('alias'),"alias","表記名")}}</div>
              </div>
              ...
              <div class="form-group">
                ...
                <select
                  ...
                  class="custom-select"
                  name="staff_type_id"
                  :class="{'is-invalid':form.errors.has('staff_type_id')}"
                >
                  <option
                    v-for="type in staff_types"
                    :key="type.id"
                    v-bind:value="type.id"
                  >{{ type.name }}</option>
                </select>
                <div
                  v-if="form.errors.has('staff_type_id')"
                  class="invalid-feedback"
                >スタッフの区分を指定してください。</div>
              </div>
              ...
              <div class="form-group">
                ...
                <select
                  ...
                  class="custom-select"
                  name="clinic_id"
                  :class="{'is-invalid':form.errors.has('clinic_id')}"
                >
                  <option
                ...
                <div
                  v-if="form.errors.has('clinic_id')"
                  class="invalid-feedback"
                >{{errormsg(form.errors.get('clinic_id'),"clinic id","クリニック名")}}</div>
              </div>
              <div class="form-group">
                <label>休退職</label>
                <select
                ...
                <div
                  ...
                  class="invalid-feedback"
                >{{errormsg(form.errors.get('is_vacation'),"is vacation","休退職")}}</div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    searchit() {
      axios
        .get("/api/staff?keyword=" + this.keyword)
        .then(({ data }) => (this.data = data));
    },
    loadList() {
      axios.get("/api/staff").then(({ data }) => (this.data = data));
      axios.get("/api/clinic").then(({ data }) => {
        this.clinics = data;
      });
      axios.get("/api/staff-type").then(({ data }) => {
