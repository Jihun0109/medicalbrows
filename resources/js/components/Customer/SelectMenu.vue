  ...
  <div class>
      ...
      <div class="row" v-show="passdata.staff_info.id !== '' && passdata.mode === 0">
        ...
        <div class="col">
          <p
            ...
            v-if="passdata.staff_info"
          >{{passdata.staff_info.name}}【{{passdata.rank_info.name}}】</p>
        </div>
      </div>
      <div class="row" v-show="passdata.staff_info.id !== '' && passdata.mode === 0">
      ...
      <div class="card">
        <div class="card-body">
          ...
          <div v-show="selectedstaff !== null">
            ...
            <div class="col-auto" style="letter-spacing: -2.5px;" v-if="selectedstaff">
              <span
    ...
    <div class="selmenu-card" v-show="passdata.staff_info.id !=='' || selectedstaff != null">
      <div class="row justify-content-between">
        ...
        <div class="col-auto">
          <button
            @click="onClickPriceBtn"
            type="button"
            class="btn btn-primary"
            style="background:#DED0B6; color:black; margin-bottom:0px;"
          >料金表</button>
        </div>
      </div>
      <div class="card">
    ...
    <div class="seldate-card" v-show="selectedmenu != null">
      ...
      <div class="smnext row justify-content-between" v-show="passdata.mode !== 1">
        <span @click="prevWeekBtn" class="col-4">
          <i v-show="nextweek_count" class="fas fa-caret-left" style="font-size:24px;"></i>
        </span>
        <span @click="nextWeekBtn" class="col-auto">
      ...
      <div class="calendar" style="margin: auto;" :class="[{oneday:passdata.mode === 1}]">
        ...
        >
          <grid-item
            @click.native="onClickCalItem($event, item, index)"
            v-for="(item, index) in passdata.calendar_layout"
            :x="item.x"
            :y="item.y"
            :w="item.w"
            :h="item.h"
    ...
    <div class="seldtime-card" v-show="sel_time_schedule !== null">
      ...
      <b-form-radio
        v-model="sel_time_schedule"
        v-for="(tc, index) in time_schedules"
        :key="index"
        :value="tc"
        name="radio-seltime"
        size="lg"
      >
        ...
        >{{passdata.clinic_info.name}}</span>
      </b-form-radio>
    </div>
    <div class="confirm-btn">
      <div class="row justify-content-around">
        ...
        <div class="col-auto" style="margin-left: 40px;">
          <button
            v-show="sel_time_schedule !== null && selectedmenu !== null"
...
export default {
  ...
  methods: {
    ...
    onClickNextBtn: function() {
      gOrderInfo.data.staff_choosed =
        this.passdata.mode === 0 ? "あり" : "なし";
    ...
    getCalendarLayout: function(staff_info, showdays, selecteddate = null) {
      ...
      this.time_schedules = null;
      axios
        .post("/v1/client/canledar_info", {
          order_type: this.passdata.order_type,
          staff_info: staff_info,
          weekmethod: showdays,
          selecteddate: selecteddate,
          count: this.nextweek_count,
          menu_info: this.selectedmenu,
          rank_info: this.passdata.rank_info
        })
        .then(({ data }) => {
          this.passdata.colNum = data.layout_width;
          this.passdata.calendar_layout = JSON.parse(
