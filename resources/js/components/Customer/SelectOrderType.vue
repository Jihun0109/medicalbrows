            ...
            <div class="radiobtns">
              <b-row>
                <b-col style="display: flex">
                  <b-form-radio
                    v-model="order_type"
                    name="radio-ordertype"
                    value="新規"
                    style="padding-right: 20px;"
                  >新規</b-form-radio>
                  <a @click="onNewHelp" data-toggle="modal" href="#modalInput">
              ...
              <b-row>
                <b-col style="display: flex">
                  <b-form-radio
                    ...
                    style="padding-right: 20px;"
                  >再診</b-form-radio>
                  <a @click="onOldHelp" data-toggle="modal" href="#modalInput">
                    <i class="help-icon fas fa-question-circle"></i>
                  </a>
                </b-col>
              </b-row>
              <label v-show="order_type === '再診'" style="margin-top: 10px;">
                前回予約IDをお持ちの方は
                <a @click="existIDPage" href="#" class="card-link">こちら</a>
              </label>
            </div>
          </b-card-text>
        </b-card>
        ...
        <b-card title>
          <b-card-text>
            <div class="radiobtns">
              <div>
                <b-form-radio
                  v-model="order_method"
                  value="staff"
                  name="radio-method"
                  size="lg"
                >施術者を優先して予約</b-form-radio>
                <select
                  v-show="order_method === 'staff'"
                ...
                <select
                  v-show="order_method === 'staff' && selectedrank !== null"
                ...
                <div v-show="selectedstaff !== null && order_method === 'staff'">
                  ...
                  <div style="letter-spacing:-1.5px;" v-if="selectedstaff">
                    <span
                      v-for="(a,i) in selectedstaff.area"
                      :key="i"
                ...
                <b-form-radio
                  ...
                  name="radio-method"
                  size="lg"
                >場所を優先して予約</b-form-radio>
                <select
                  v-show="order_method === 'clinic'"
                ...
                <label
                  v-show="order_method === 'clinic' && order_type ==='再診'"
                  style="margin-left: 10px; width:65%"
                >※院変更の場合は 別途 初診料2,000円（税別）が発生します</label>
              </div>
            </div>
          </b-card-text>
        </b-card>
        <label
          v-show="order_type === '再診'&& order_method === 'staff'"
          style="margin-left: 10px; width:65%"
        >※ランクアップ変更の場合は 別途 追加費用が発生します</label>
      </div>
      <div class="confirm-btn">
        <div class="row justify-content-around">
          ...
          <div class="col-auto" style="margin-left: 40px;">
            <button
              v-show="showNextBtn(order_method)"
...
export default {
  ...
  methods: {
    ...
    onClickNextBtn: function() {
      ...
      if (this.selectedstaff) {
        ...
        gOrderTypeInfo.data.date = moment(this.selecteddate).format(
          "YYYY-MM-DD"
        );
        ...
        axios
          .post("/v1/client/staff_list", { clinic_info: this.selectedclinic })
          .then(({ data }) => {
            gOrderTypeInfo.data.staffs = data;
            this.$emit("changeStage", 1);
            console.log(
    ...
    staff_rank_List: function() {
      axios
        .post("/v1/client/staff_list", { rank_info: this.selectedrank })
        .then(({ data }) => {
          this.staffs = data;
    ...
    menu_List: function(curdate) {
      axios
        .post("/v1/client/menu_list", {
          staff_info: this.selectedstaff,
  ...
  watch: {
    ...
    selectedrank(val) {
      this.selectedstaff = null;
      if (this.selectedrank) this.staff_rank_List();
    }
    // selectedstaff(val) {
    //     if(this.selectedstaff)
    //         this.menu_List();
